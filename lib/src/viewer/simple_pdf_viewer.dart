...
class SimplePdfViewer extends StatelessWidget {
  ...
  PdfViewerParams _buildParams(bool isDark, Color bgColor) {
    return PdfViewerParams(
      ...
      pageDropShadow: showPageShadow
          ...
          : null,
      selectableRegionInjector: enableTextSelection
          ? (context, child) => ReaderSelectionGuard(child: child)
          : null,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final viewer = switch (_sourceType) {
      _SourceType.file => PdfViewer.file(
          _filePath!,
          controller: controller,
          params: _buildParams(isDark, bgColor),
        ),
      _SourceType.data => PdfViewer.data(
          _data!,
          sourceName: _sourceName!,
          controller: controller,
          params: _buildParams(isDark, bgColor),
        ),
      _SourceType.uri => PdfViewer.uri(
          _uri!,
          controller: controller,
          params: _buildParams(isDark, bgColor),
        ),
